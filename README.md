...
実装したのは要件の3点(インターン生の登録 / 企業からのメッセージ / 募集の掲載)だけです。
機能を増やすより、なぜそう作ったかを説明できる状態にすることを優先しました。
...
テストは RSpec が 70 examples、Vitest が 8 tests。
...
おかげで JWT のペイロードは `account_id` だけで済み、トークンを検証する側はロールを知らずにいられます
(`app/services/json_web_token.rb`, `app/controllers/concerns/authenticatable.rb`)。

代償として、プロフィールを触るたびに `current_account.profileable` を一段挟むことになります。
この規模なら気にならない範囲です。
...
(`frontend/src/lib/auth.tsx`)。XSS を踏んだら盗まれる場所なので、本番なら httpOnly Cookie か
BFF 経由にすべきところです。
...
```bash
docker compose exec backend bundle exec rspec    # 70 examples, 0 failures
docker compose exec frontend npm run test        # 8 passing
...
フロントは API クライアント、認証コンテキスト、登録フォーム、募集の削除フロー、それと `useApi` フックのテスト。
...
- 検索と絞り込みは、インターン一覧にも募集一覧にも入れていません
- 募集を見た学生から企業に応募する導線がなく、スカウトは企業からの一方向だけです
- 既読は API を叩いたときに更新されるだけで、WebSocket などでの即時反映はしていません
- 企業のプロフィール編集がありません。インターン生は `PATCH /interns/me` で編集できます。
  設計時は入れるつもりでしたが、優先度を下げました
- `frontend/src/types` は手書きです。本来は OpenAPI から生成してズレを防ぎたいところ
- JWT の保存場所は localStorage のままです(前述)

レート制限のカウンタは `MemoryStore` に置いているので、プロセスの中にしかありません。
今は Puma もコンテナも1つなので問題ないですが、ワーカーを増やした時点で
カウンタが分裂して制限が実質ゆるくなります。増やす前に Redis に移す必要があります。
